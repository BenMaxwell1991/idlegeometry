...
pub fn initialise_data(game_data: Arc<GameData>) {
    ...
    init_attacks(Arc::clone(&game_data));
    println!("Initialised Attacks");

    init_lair(Arc::clone(&game_data));
...
fn init_attacks(game_data: Arc<GameData>) {
    let pool_config = vec![
...
fn init_lair(game_data: Arc<GameData>) {
    let mut player_data = acquire_lock_mut(&game_data.player_data, "player_data");
    if player_data.lair_objects.is_empty() {
        player_data.lair_objects = PlayerData::initialize_lair_objects(Arc::clone(&game_data));
...
pub fn initialise_attack_pools(game_data: Arc<GameData>, pool_sizes: &[(AttackName, usize)]) {
