...
pub fn show_game_menu_lair(ui: &mut Ui, game_data: &GameData, game_rect: Rect) {
    let button_size = Vec2::new(250.0, 50.0);
    let bottom_center = Pos2::new(
        game_rect.center().x - button_size.x / 2.0,
        game_rect.max.y - button_size.y - 20.0,
    );
    let menu_rect = Rect::from_min_size(bottom_center, button_size);
