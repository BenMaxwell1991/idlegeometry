...
pub fn show_geometry(ui: &mut egui::Ui, game: Arc<Mutex<Game>>, game_clone: &Game) {
    ...
    ui.with_layout(Layout::left_to_right(Align::Min), |ui| {
        ui.columns(2, |columns| {
            columns[0].add(CustomGrid::new(&game_clone, grid_id));
            columns[1].add(GameGraphics::new(Arc::clone(&game), graphics_id));
        });
    });
